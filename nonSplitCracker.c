/*
	...
	Auteurs:
		- CAMBERLIN Merlin
		- PISVIN Arthur
	Version: 07-05-19 - Considération du nombre de threads max si il y a plusieurs fichiers binaires d'entrée
...
int main(int argc, char *argv[]) 
{
	...
	for(int i=0; i<nbreFichiersEntree; i++)
	{
		...
		if(stat(fichiersEntree[i], &stat_fichier)==-1)
		{
			fprintf(stderr, "Erreur stat fichier\n");
			return EXIT_FAILURE;
		}
		nbre_de_hash += (stat_fichier.st_size)/32;
	}
	...
	printf("le nombre de threads de calcul est %d\n", nbreThreadsCalcul);
