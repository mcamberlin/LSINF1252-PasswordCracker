/*
	Fichier crackerMerlin.c réprésente le code dans son intégralité

	Auteurs:
		- CAMBERLIN Merlin
		- PISVIN Arthur
	Version: 05-05-19 - Copie conforme de cracker.c avec objectif d'implémenter le cas avec plusieurs fichiers d'entée.

	Commandes à indiquer dans le shell:
		- cd ~/Documents/LSINF1252-PasswordCracker-Gr118-2019
		- gcc crackerMerlin.c reverse.c sha256.c -o cracker -lpthread -Wall -Werror
...
/** La fonction lectureFichier() lis par 32 bytes le fichier binaire et rempli au fur et à mesure la ressource @tab_hash de hash. 
	@pre - 
...
int main(int argc, char *argv[]) 
{
	...
	// Initialisation des cellules du tableaux d'adresse à NULL pour plus tard vérifier si une case est NULL.
	for(int i=0; i<N; i++)
	{
		*(tab_mdp+i)=NULL;
	}
